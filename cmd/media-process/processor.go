...
func processFile(ctx context.Context, key string) error {
	...
	mimeType, _ := media.GetMIMEType(ext)
	if mimeType == "" {
	...
	mf, err := media.LoadMediaFile(localPath)
	if err != nil {
	...
	if isImage {
		...
		thumbData, _, err := media.GenerateThumbnail(mf, thumbnailPx)
		if err != nil {
		...
		if media.IsFFmpegAvailable() {
			...
			if mf.Metadata != nil {
				if vm, ok := mf.Metadata.(*media.VideoMetadata); ok {
					videoMeta = vm
				}
			}
			...
			if videoMeta != nil && videoMeta.Duration > 0 {
				preset := media.SelectPreset(videoMeta.Duration)
			...
			compressedPath, _, cleanup, err := media.CompressVideoForGemini(compressCtx, localPath, videoMeta)
			compressCancel()
			if err != nil {
