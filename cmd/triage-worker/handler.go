...
func handleTriageRun(ctx context.Context, event TriageEvent) (interface{}, error) {
	...
	for _, fr := range validFiles {
		...
		if mimeType == "" {
			mimeType, _ = media.GetMIMEType(strings.ToLower(filepath.Ext(fr.Filename)))
		}
		...
		if media.IsImage(ext) && fr.ThumbnailKey != "" {
			useKey = fr.ThumbnailKey
			if m, _ := media.GetMIMEType(strings.ToLower(filepath.Ext(fr.ThumbnailKey))); m != "" {
				mimeType = m
			}
		}
	...
	if ebClient != nil {
		...
		for _, tr := range triageResults {
			...
			mediaType := "Photo"
			if ext := strings.ToLower(filepath.Ext(tr.Filename)); media.IsVideo(ext) {
				mediaType = "Video"
			}
