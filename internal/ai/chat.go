...
func AskMediaQuestion(ctx context.Context, client *genai.Client, mediaFile *media.MediaFile, question string) (string, error) {
	mediaType := "media"
	...
	if media.IsVideo(ext) {
		...
		if mediaFile.Metadata != nil {
			videoMeta, _ = mediaFile.Metadata.(*media.VideoMetadata)
		}

		compressedPath, compressedSize, cleanup, err := media.CompressVideoForGemini(ctx, mediaFile.Path, videoMeta)
		if err != nil {
...
func uploadAndWaitForProcessing(ctx context.Context, client *genai.Client, mediaFile *media.MediaFile) (*genai.File, error) {
	log.Debug().
		Str("path", mediaFile.Path).
		Int64("size_bytes", mediaFile.Size).
		Str("mime_type", mediaFile.MIMEType).
...
func AskImageQuestion(ctx context.Context, client *genai.Client, mediaFile *media.MediaFile, question string) (string, error) {
...
func BuildSocialMediaPrompt(metadata media.MediaMetadata) string {
	if metadata == nil {
