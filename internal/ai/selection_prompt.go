...
func BuildMediaSelectionPrompt(files []*media.MediaFile, maxItems int, tripContext string) string {
...
func BuildPhotoSelectionPrompt(files []*media.MediaFile, maxPhotos int, tripContext string) string {
	var sb strings.Builder
	...
	for i, file := range files {
		...
		if file.Metadata != nil {
			...
			if imgMeta, ok := file.Metadata.(*media.ImageMetadata); ok {
...
func writeMediaMetadata(sb *strings.Builder, files []*media.MediaFile) {
	for i, file := range files {
		ext := strings.ToLower(filepath.Ext(file.Path))
		mediaType := "Photo"
