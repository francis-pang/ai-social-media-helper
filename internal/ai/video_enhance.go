...
func EnhanceVideo(ctx context.Context, videoPath string, outputPath string, metadata *media.VideoMetadata, config VideoEnhancementConfig) (*VideoEnhancementResult, error) {
	startTime := time.Now()
	...
	if config.SimilarityThreshold <= 0 {
		config.SimilarityThreshold = media.DefaultSimilarityThreshold
	}
	...
	extraction, err := media.ExtractFrames(ctx, videoPath, metadata)
	if err != nil {
	...
	groups, err := media.GroupFramesByHistogram(extraction.FramePaths, config.SimilarityThreshold)
	if err != nil {
	...
	for i, group := range groups {
		...
		if result.lutContent != "" {
			...
			err = media.ApplyLUTToFrames(ctx, group.FramePaths, result.lutContent, enhancedFrameDir)
			if err != nil {
	...
	err = media.ReassembleVideo(ctx, enhancedFrameDir, videoPath, outputPath, extraction.ExtractionFPS)
	if err != nil {
