...
func enhanceFrameGroup(ctx context.Context, geminiClient *GeminiImageClient, imagenClient *ImagenClient, group media.FrameGroup, groupIndex int, config VideoEnhancementConfig) (*internalGroupResult, error) {
	log.Debug().
	...
	lutContent, err := media.ComputeColorLUT(group.RepresentativePath, enhancedRepPath)
	if err != nil {
		log.Warn().Err(err).Int("group", groupIndex).
...
func copyGroupFrames(group media.FrameGroup, outputDir string) error {
